...
export default async function AdminAnalyticsPage() {
  ...
  const eventRows = topEvents.map((e: typeof topEvents[number]) => {
    const gross = e.orders.reduce((s: number, o: typeof e.orders[number]) => s + Number(o.total), 0);
    const fees = e.orders.reduce((s: number, o: typeof e.orders[number]) => s + Number(o.platformFee), 0);
  ...
  const orgRows = topOrganizers.map((org: typeof topOrganizers[number]) => {
    const allOrders = org.events.flatMap((e: typeof org.events[number]) => e.orders);
    const gross = allOrders.reduce((s: number, o: typeof allOrders[number]) => s + Number(o.total), 0);
    const fees = allOrders.reduce((s: number, o: typeof allOrders[number]) => s + Number(o.platformFee), 0);
